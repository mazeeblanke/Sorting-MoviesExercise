...
window.onload = function() {
    const sortedMovies = sortMoviesByAttr(movies, 'title')
    // Display Movies list
...
function sortMoviesByAttr(movies, sortAttr){
  ...
  for (let j = 0; j < movies.length - 1; j++) {
    ...
    const {max_movie: max_obj, max_index: max_location} = getMaxMovieObject(movies, j, sortAttr)
